...
class CoppeliaSim:
    ...
    def __init__(
        ...
        pose_tolerance: tuple[float, float],
    ):
        ...
        Args:
            ...
            pose_tolerance: Position [m] and angle [º] error thresholds.
    ...
    ) -> tuple[tuple[float, float, float], float, float | None, bool]:
        """_summary_

        Args:
            x: x world coordinate [m].
            y: y world coordinate [m].
            theta: Orientation around the z axis [rad]. Optional.

        Returns:
            real_pose: Real robot pose (x, y, theta) [m, m, rad].
            position_error: Distance between the query point and the true robot location [m].
            angle_error: Difference between the real and the predicted orientations [º].
                Computed only if theta is provided.
            within_tolerance: True if the robot center is within the goal and orientation tolerance.
                If no predicted angle is provided, only the position is considered.
